...
checkBtn.onclick = function(){
    ...
    tryCount++;
    if (userAnswer === "q") {
        hide(checkBtn)//добавим класс в #button
        hide(input);
        show(repeatBtn);
        changeMessage(`We'll play another time`)
        ...
        hide(input);
        show(repeatBtn);
        changeMessage(`You lost( <br> correct answer ${guessNumber}`)
    } else if (userAnswer > guessNumber) {
        changeMessage(`The number is to big, <br> try again. You have ${MAX_TRY_COUNT - tryCount} more attempts`);
        input.value = "";
    } else {
        changeMessage(`The number is to small, <br> try again. You have ${MAX_TRY_COUNT - tryCount} more attempts`);
        input.value = "";
    }
}
