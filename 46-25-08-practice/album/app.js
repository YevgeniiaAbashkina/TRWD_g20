...
function renderUser(user) {
  ...
  element.innerHTML = `<h2>${user.name}</h2>
...
function userClickHandler (event){
  left.innerHTML = "";
  right.innerHTML = "";
  //console.log(event.target)//попадает в клик и див, и заголок, и абзац
  //console.log(event.currentTarget)//при клике на детей будет записываться клик на родителе
...
const getAlbumsByUserId = (userId) => {
  const element = document.createElement("div");
  const userAlbums = albums.filter(element => element.userId === userId);
...
function renderAlbum(album) {
  ...
  elementAlbum.id = "album_" + album.id;
  elementAlbum.innerHTML = `<h3>${album.title}</h3>`;
  elementAlbum.onclick = albumClickHandler;
  return elementAlbum;
}

function albumClickHandler(event) {
  //console.log(event.currentTarget)
  right.innerHTML = "";
  const albumId = +event.currentTarget.id.split("_")[1];//album_1
  //console.log(userId);
  right.append(getFotosByAlbumId(albumId));
}

function getFotosByAlbumId(id) {
  const elemFoto = document.createElement("div");
  const photosByAlbumId = photos.filter(photo => photo.albumId === id);
  if (!photosByAlbumId.length) {
    elemFoto.innerHTML = "no photos";
    return elemFoto;
  }
  photosByAlbumId.forEach(photo => {
    elemFoto.append(renderPhoto(photo))
  })
  elemFoto.style.display = "flex";
  elemFoto.style.flexWrap = "wrap";
  return elemFoto;
};
